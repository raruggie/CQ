...
df.NWIS.Q.for_NO3<-readNWISdv(siteNumbers = df.NWIS.NO3_sites$site_no, parameterCd = '00060', startDate = "", endDate = "", statCd = "00003")

# write.csv(df.NWIS.Q.for_NO3, "Raw_Data/df.NWIS.Q.for_NO3.csv", row.names=FALSE)

df.NWIS.Q.for_NO3<-read.csv("Raw_Data/df.NWIS.Q.for_NO3.csv", colClasses = c(site_no = "character"))

# download the raw discrete TP sample data:

df.NWIS.NO3<-readNWISqw(siteNumbers = df.NWIS.NO3_sites$site_no, parameterCd = '00600')

# write.csv(df.NWIS.NO3, "Raw_Data/df.NWIS.NO3.csv", row.names=FALSE)

df.NWIS.NO3<-read.csv("Raw_Data/df.NWIS.NO3.csv", colClasses = c(site_no = "character"))
...
# how many of these NO3 sites overlap with the top 40 TP sites?
# cause maybe I dont need to delineate all of them:

load('Processed_Data/Top_40_TP_sites.Rdata')

x<-filter(NO3_sites, site_no %in% m$site_no)


p<-filter(df_Seg, site %in% x$site_no)%>%
...
save.image(file = 'Processed_Data/NWIS_NO3.Rdata')
