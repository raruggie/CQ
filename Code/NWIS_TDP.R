...
df.NWIS.Q.for_TDP<-readNWISdv(siteNumbers = df.NWIS.TDP_sites$site_no, parameterCd = '00060', startDate = "", endDate = "", statCd = "00003")

# write.csv(df.NWIS.Q.for_TDP, "Raw_Data/df.NWIS.Q.for_TDP.csv", row.names=FALSE)

df.NWIS.Q.for_TDP<-read.csv("Raw_Data/df.NWIS.Q.for_TDP.csv", colClasses = c(site_no = "character"))
...
df.NWIS.TDP<-read.csv("Raw_Data/df.NWIS.TDP.csv", colClasses = c(site_no = "character"))
...
df.NWIS.TDP_CQ_for_BP<-df.NWIS.TDP_CQ%>%
  drop_na(result_va, Q_yield)

# create a vector of ordered unique site names:

temp.loop<-sort(unique(df.NWIS.TDP_CQ_for_BP$site_no))
...
# filter the gauges2 to the NYS TDP CQ sites:

df.G2<-df.G2%>%filter(STAID %in% TDP_sites$site_no)
